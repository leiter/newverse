...
class GitLiveProfileRepository(
    ...
    // private val sellersRef = database.reference("profiles/sellers")

    // Local state management
    private val _buyerProfile = MutableStateFlow<BuyerProfile?>(null)
    private val buyerProfileFlow: Flow<BuyerProfile?> = _buyerProfile.asStateFlow()

    // Cache for seller profiles
    ...
    override fun observeBuyerProfile(): Flow<BuyerProfile?> {
        ...
        //     if (userId != null) {
        //         buyersRef.child(userId).valueEvents.collect { snapshot ->
        //             val profile = snapshot.value<BuyerProfileDto>()?.toDomain()
        //             _buyerProfile.value = profile
        //         }
        //     }
        // }

        return buyerProfileFlow
    }
    ...
    override suspend fun getBuyerProfile(): Result<BuyerProfile> {
        return try {
            ...
            println("✅ GitLiveProfileRepository.getBuyerProfile: Created default profile for $userId")
    ...
    override suspend fun saveBuyerProfile(profile: BuyerProfile): Result<BuyerProfile> {
        return try {
            ...
            println("✅ GitLiveProfileRepository.saveBuyerProfile: Success - saved to cache")
    ...
    override suspend fun saveSellerProfile(profile: SellerProfile): Result<Unit> {
        return try {
            ...
            println("✅ GitLiveProfileRepository.saveSellerProfile: Success - saved to cache")
            Result.success(Unit)
    ...
    override suspend fun clearUserData(sellerId: String, buyerProfile: BuyerProfile): Result<Boolean> {
        return try {
            ...
            println("✅ GitLiveProfileRepository.clearUserData: Success - cleared local data")
            Result.success(true)
