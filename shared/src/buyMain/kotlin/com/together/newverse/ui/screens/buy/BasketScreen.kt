...
fun BasketContent(
    ...
    if (state.showDatePicker) {
        DatePickerDialog(
            ...
            onDateSelected = { date ->
                onAction(BuyBasketScreenAction.SelectPickupDate(date))
            },
            onDismiss = { onAction(BuyBasketScreenAction.HideDatePicker) }
        )
    }
    ...
    if (state.showReorderDatePicker) {
        ReorderDatePickerDialog(
            ...
            onDateSelected = { date ->
                onAction(BuyBasketScreenAction.ReorderWithNewDate(date, currentArticles))
            },
            onDismiss = {
                onAction(BuyBasketScreenAction.HideReorderDatePicker)
            }
        )
    }
    ...
    if (state.showMergeDialog && state.existingOrderForMerge != null) {
        OrderMergeDialog(
            ...
            onConfirm = { onAction(BuyBasketScreenAction.ConfirmMerge) },
            onDismiss = { onAction(BuyBasketScreenAction.HideMergeDialog) }
        )
    }
    ...
    if (state.showDraftWarningDialog) {
        DraftWarningDialog(
            ...
            onSaveAndContinue = { onAction(BuyBasketScreenAction.SaveDraftAndLoadOrder) },
            onDiscardAndContinue = { onAction(BuyBasketScreenAction.DiscardDraftAndLoadOrder) },
            onCancel = { onAction(BuyBasketScreenAction.HideDraftWarningDialog) }
        )
    }
