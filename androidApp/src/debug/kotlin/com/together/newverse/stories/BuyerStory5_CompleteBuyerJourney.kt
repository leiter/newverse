...
suspend fun runBuyerStory5_CompleteBuyerJourney(
    ...
    var currentState = buyAppViewModel.state.value.screens.mainScreen
    ...
    articlesToFavorite.forEach { article ->
        ...
        buyAppViewModel.dispatch(UnifiedMainScreenAction.ToggleFavourite(article.id))
        delay(1000)
    }

    currentState = buyAppViewModel.state.value.screens.mainScreen
    ...
    buyAppViewModel.dispatch(UnifiedMainScreenAction.SelectArticle(firstItem))
    delay(1200)

    println("\n[${formatElapsedTime(startTime)}] User: Enters quantity")
    val firstQuantity = 2.0
    println("   Typing: $firstQuantity ${firstItem.unit}")
    buyAppViewModel.dispatch(UnifiedMainScreenAction.UpdateQuantity(firstQuantity))
    delay(1000)
    ...
    buyAppViewModel.dispatch(UnifiedMainScreenAction.SelectArticle(articleToRemove))
    delay(500)
    buyAppViewModel.dispatch(UnifiedMainScreenAction.RemoveFromBasket)
    delay(1500)
    ...
    if (availableArticles.size > 4) {
        ...
        buyAppViewModel.dispatch(UnifiedMainScreenAction.SelectArticle(additionalItem))
        delay(800)
        buyAppViewModel.dispatch(UnifiedMainScreenAction.UpdateQuantity(1.0))
        delay(600)
        buyAppViewModel.dispatch(UnifiedMainScreenAction.AddToCart)
        delay(1500)
    ...
    val finalProfile = buyAppViewModel.state.value.screens.customerProfile.profile
    ...
    if (availableArticles.size > 5) {
        ...
        buyAppViewModel.dispatch(UnifiedMainScreenAction.SelectArticle(extraItem))
        delay(600)
        buyAppViewModel.dispatch(UnifiedMainScreenAction.UpdateQuantity(1.0))
        delay(500)
        buyAppViewModel.dispatch(UnifiedMainScreenAction.AddToCart)
    ...
    val finalState = buyAppViewModel.state.value.screens.mainScreen
