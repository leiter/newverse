...
suspend fun runBuyerStory3_CheckoutNewOrder(
    ...
    val mainState = buyAppViewModel.state.value.mainScreen
    ...
    itemsToAdd.forEach { (article, quantity, description) ->
        ...
        val currentBasket = buyAppViewModel.state.value.mainScreen.basketItems
        println("      ✓ Added. Cart now has ${currentBasket.size} items")
    }

    val preCheckoutBasket = buyAppViewModel.state.value.mainScreen.basketItems
    ...
    val afterDatesState = buyAppViewModel.state.value.basketScreen
    ...
    buyAppViewModel.dispatch(BuyBasketScreenAction.SelectPickupDate(selectedPickupDate))
    delay(800)

    val afterSelectDateState = buyAppViewModel.state.value.basketScreen
    ...
    println("   User State: ${buyAppViewModel.state.value.user}")
