...
suspend fun runBuyerStory4_EditExistingOrder(
    ...
    val availableArticles = buyAppViewModel.state.value.mainScreen.articles.filter { it.available }
    ...
    val availableDates = buyAppViewModel.state.value.basketScreen.availablePickupDates
    ...
    buyAppViewModel.dispatch(BuyBasketScreenAction.SelectPickupDate(pickupDate))
    delay(800)
    ...
    buyAppViewModel.dispatch(BuyBasketScreenAction.LoadOrder(createdOrderId, pickupDateKey))

    println("\n[WAITING] Loading order data (generous 3s delay)")
    delay(3000)

    val afterLoadState = buyAppViewModel.state.value.basketScreen
    ...
    buyAppViewModel.dispatch(BuyBasketScreenAction.UpdateItemQuantity(firstItem.productId, newQuantity))
    delay(1000)

    val afterQuantityChangeState = buyAppViewModel.state.value.basketScreen
    ...
    buyAppViewModel.dispatch(BuyMainScreenAction.SelectArticle(newArticle))
    delay(500)
    buyAppViewModel.dispatch(BuyMainScreenAction.UpdateQuantity(newItemQuantity))
    delay(500)
    buyAppViewModel.dispatch(BuyMainScreenAction.AddToCart)
    delay(1500)

    val afterAddState = buyAppViewModel.state.value.basketScreen
    ...
    buyAppViewModel.dispatch(BuyBasketScreenAction.RemoveItem(itemToRemove.productId))
    delay(1000)

    val afterRemoveState = buyAppViewModel.state.value.basketScreen
    ...
    val beforeUpdateState = buyAppViewModel.state.value.basketScreen
    ...
    if (canEdit && afterUpdateState.orderError == null) {
        ...
        val reloadedState = buyAppViewModel.state.value.basketScreen
